...
# Definir os valores mínimo e máximo dos atributos
limits = {
...
# Estilo para aumentar a fonte dos labels
st.markdown("""
    <style>
...
# Entradas do usuário
valor_quitacao = st.slider(
    "**Valor de Parcela**", 
    min_value=limits["ValorQuitacao"][0], 
    max_value=limits["ValorQuitacao"][1], 
    value=(limits["ValorQuitacao"][0] + limits["ValorQuitacao"][1]) / 2, 
    step=0.01
)
atraso = st.slider(
    "**Nº de Parcelas**", 
    min_value=limits["Atraso"][0], 
    max_value=limits["Atraso"][1], 
    value=(limits["Atraso"][0] + limits["Atraso"][1]) / 2, 
    step=1.0
)
quant_pagamentos_via_boleto = st.slider(
    "**Quant. de Boletos pagos**", 
    min_value=limits["Quant_Pagamentos_Via_Boleto"][0], 
    max_value=limits["Quant_Pagamentos_Via_Boleto"][1], 
    value=(limits["Quant_Pagamentos_Via_Boleto"][0] + limits["Quant_Pagamentos_Via_Boleto"][1]) // 2, 
    step=1
)
quant_ocorrencia = st.slider(
    "**Idade do Cliente**", 
    min_value=limits["Quant_Ocorrencia"][0], 
    max_value=limits["Quant_Ocorrencia"][1], 
    value=(limits["Quant_Ocorrencia"][0] + limits["Quant_Ocorrencia"][1]) // 2, 
    step=1
)
...
# AQUI é onde estava o problema! Ajustado:
if hasattr(encoder, 'get_feature_names_out'):
    feature_names = encoder.get_feature_names_out(["UF"])
else:
    feature_names = encoder.get_feature_names(["UF"])

uf_encoded_user_df = pd.DataFrame(uf_encoded_user, columns=feature_names)
...
# Reindexar para ter as mesmas colunas esperadas pelo modelo
